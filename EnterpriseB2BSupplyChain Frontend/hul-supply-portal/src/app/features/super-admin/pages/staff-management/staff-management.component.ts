...
@Component({
  ...
  template: `
    <div class="page-container">
      <hul-page-header title="Staff Management" subtitle="Create and manage admin accounts"></hul-page-header>
  ...
  styles: [`
    ...
    .tab-count { background: var(--hul-primary-light); color: var(--hul-primary); border-radius: 9999px; font-size: 11px; font-weight: 700; padding: 1px 7px; }
...
export class StaffManagementComponent implements OnInit {
  ...
  allStaff: any[] = [];
  showForm = false;
  form = { fullName: '', email: '', password: '', phoneNumber: '' };
  ...
  canSubmit(): boolean {
    return !!(this.form.fullName?.trim() && this.form.email?.trim() && this.form.password?.length >= 8);
  }

  createStaff(): void {
    const payload = { ...this.form, role: 'Admin' };
    this.http.post(API_ENDPOINTS.superAdmin.createAdmin(), payload).subscribe({
      next: () => {
        this.toast.success(`Admin account created for ${this.form.fullName}`);
        this.showForm = false;
        this.form = { fullName: '', email: '', password: '', phoneNumber: '' };
        this.load();
      },
