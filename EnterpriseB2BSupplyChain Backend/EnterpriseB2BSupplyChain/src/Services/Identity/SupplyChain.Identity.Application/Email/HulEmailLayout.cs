namespace SupplyChain.Identity.Application.Email;

/// <summary>
/// Industry-standard HTML email layout for the HUL Supply Chain Platform.
///
/// All emails sent from the Identity service should be wrapped in <see cref="Wrap"/>
...
public static class HulEmailLayout
{
    ...
    /// </summary>
    /// <param name="title">Title shown in the header bar (e.g. "Account Verification").</param>
    /// <param name="bodyHtml">The inner content HTML to render inside the card.</param>
    /// <param name="preheader">Hidden preview text shown by Gmail/Outlook in the inbox list.</param>
    /// <param name="accentColor">Header bar color. Defaults to HUL primary blue.</param>
    ...
    /// <summary>Greeting paragraph (e.g. "Hello, Srikaran,").</summary>
    ...
    /// <summary>
    /// Large coloured callout used for OTP codes, order numbers, etc.
    /// </summary>
    public static string CodeCallout(string code, string label = "Verification Code")
        => $$"""
            <div style="margin:24px 0;padding:24px;background-color:#f0f9ff;border:2px dashed {{Primary}};border-radius:12px;text-align:center;">
              <div style="font-size:11px;font-weight:700;color:{{Primary}};letter-spacing:1.5px;text-transform:uppercase;margin-bottom:10px;">{{label}}</div>
              <div style="font-size:34px;font-weight:700;color:{{TextPrimary}};letter-spacing:8px;font-family:'Courier New',Consolas,monospace;">{{code}}</div>
              <div style="font-size:12px;color:{{TextMuted}};margin-top:10px;">Expires in 15 minutes</div>
            </div>
            """;

    /// <summary>Coloured info box (success / warning / danger / info).</summary>
    ...
    /// <summary>Primary CTA button (table-based for Outlook compatibility).</summary>
            ...
            <table role="presentation" border="0" cellpadding="0" cellspacing="0" style="margin:24px 0;">
              <tr>
                <td align="center" style="border-radius:8px;background-color:{{btnColor}};">
                  <a href="{{href}}" target="_blank" style="display:inline-block;padding:13px 32px;font-size:14px;font-weight:600;color:#ffffff;text-decoration:none;border-radius:8px;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;">{{Escape(label)}}</a>
                </td>
              </tr>
            </table>
            """;
    ...
    /// <summary>Closing block: "Regards, HUL Supply Chain Team".</summary>
    ...
    /// <summary>Minimal HTML escape for user-supplied text values.</summary>
