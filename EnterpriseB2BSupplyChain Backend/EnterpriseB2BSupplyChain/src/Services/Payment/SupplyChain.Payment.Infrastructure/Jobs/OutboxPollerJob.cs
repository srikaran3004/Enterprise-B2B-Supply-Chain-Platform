...
namespace SupplyChain.Payment.Infrastructure.Jobs;
...
public class OutboxPollerJob
{
    ...
    public async Task ExecuteAsync()
    {
        ...
        foreach (var message in messages)
        {
            try
            {
                var routingKey = $"payment.{message.EventType.ToLowerInvariant()}";
                ...
                _logger.LogInformation(
                    "Published payment outbox message {MessageId} — {EventType} → {RoutingKey}",
