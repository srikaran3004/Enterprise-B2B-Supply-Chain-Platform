...
{
    private const string ConsumerName = "payment-order-delivered-consumer";
    ...
    private string _queue = "payment.order-delivered.queue";
    private string _routingKey = "order.delivered";
    private string _deadLetterExchange = "supplychain.domain.events.dead";
    private string _deadLetterQueue = "payment.order-delivered.queue.dead";
    private string _deadLetterRoutingKey = "payment.order-delivered.queue.dead";
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        ...
        _queue = _configuration["RabbitMQ:OrderDeliveredQueue"] ?? "payment.order-delivered.queue";
        _routingKey = _configuration["RabbitMQ:OrderDeliveredRoutingKey"] ?? "order.delivered";
        ...
        _logger.LogInformation(
            "OrderDelivered consumer started. Queue: {Queue}, RoutingKey: {RoutingKey}, DLQ: {DeadLetterQueue}, MaxRetries: {MaxRetries}",
    ...
    private async Task HandleMessageAsync(BasicDeliverEventArgs args, CancellationToken ct)
    {
        ...
        try
        {
            if (!eventType.Equals("OrderDelivered", StringComparison.OrdinalIgnoreCase))
            {
            ...
            {
                _logger.LogWarning(
                    "Skipping OrderDelivered event due to missing OrderId. MessageId={MessageId}",
                    messageId);
                await _channel.BasicAckAsync(args.DeliveryTag, false, ct);
                return;
            }
            ...
            var orderClient = scope.ServiceProvider.GetRequiredService<IOrderInternalClient>();
            var mediator = scope.ServiceProvider.GetRequiredService<IMediator>();

            var invoiceDetails = await orderClient.GetInvoiceDetailsAsync(orderId.Value, ct);
            if (invoiceDetails is null)
                throw new InvalidOperationException($"Could not load invoice details for order {orderId.Value}.");

            var originState = _configuration["Invoice:OriginState"];
            var isInterstate = !string.IsNullOrWhiteSpace(originState)
                && !string.IsNullOrWhiteSpace(invoiceDetails.ShippingState)
                && !string.Equals(originState, invoiceDetails.ShippingState, StringComparison.OrdinalIgnoreCase);
            ...
            var command = new GenerateInvoiceCommand(
                ...
                Lines: invoiceDetails.Lines.Select(line => new InvoiceLineRequest(
                    ProductId: line.ProductId,
                    ProductName: line.ProductName,
                    SKU: line.SKU,
                    Quantity: line.Quantity,
                    UnitPrice: line.UnitPrice)).ToList());
        ...
        catch (Exception ex)
        {
            ...
            if (nextRetry > _maxRetries)
            {
                ...
                _logger.LogError(ex,
                    "Payment event moved to DLQ. EventType={EventType}, MessageId={MessageId}, Retries={Retries}",
            ...
            else
            {
                ...
                _logger.LogWarning(ex,
                    "Payment event retry queued. EventType={EventType}, MessageId={MessageId}, Retry={Retry}/{MaxRetries}",
