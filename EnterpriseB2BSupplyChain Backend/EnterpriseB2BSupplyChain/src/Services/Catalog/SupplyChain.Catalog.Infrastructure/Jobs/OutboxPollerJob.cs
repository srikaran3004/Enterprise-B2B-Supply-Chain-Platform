    ...
    public async Task ExecuteAsync()
    {
        ...
        foreach (var message in messages)
        {
            try
            {
                ...
                {
                    "StockRestored" => "catalog.stock.restored",
                    _               => $"catalog.{message.EventType.ToLowerInvariant()}"
                };
                ...
                _logger.LogInformation(
                    "Published catalog outbox message {MessageId} — {EventType} → {RoutingKey}",
            ...
            catch (Exception ex)
            {
                ...
                _logger.LogWarning(ex,
                    "Failed to publish catalog outbox message {MessageId}", message.MessageId);
            }
        }
